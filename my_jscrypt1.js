...
function setKey(incoming_key) {
	var s = encode_utf8(incoming_key);
	var i, kmd5e, kmd5o;
...
function Encrypt_text() {
	...
	var incoming_text = document.plain.text.value
	var incoming_key  = document.key.text.value

	if (incoming_key.length == 0) {
...
//	document.cipher.text.value = "";
	setKey(document.key.text.value);

	addEntropyTime();
	prng = new AESprng(keyFromEntropy());
	var plaintext = encode_utf8(incoming_text);
...
alert("Failure to determine Armour Type");
		if (document.plain.encoding[0].checked) {
			kt = 0;
		} else if (document.plain.encoding[1].checked) {
			kt = 1;
...
function Decrypt_text() {
	...
	var incoming_key  = document.key.text.value
	var incoming_text = document.cipher.text.value
...
//	document.plain.text.value = "";
	setKey(document.key.text.value);
	var ct = new Array(), kt;
	kt = determineArmourType(incoming_text);
	if (kt == 0) {
			ct = disarm_codegroup(incoming_text);
	} else if (kt == 1) {
			ct = disarm_hex(incoming_text);
	} else if (kt == 2) {
			ct = disarm_base64(incoming_text);
	}
